...
| `textAlign` | ⚠️ | 4/5/2/5 | +17 | 2026-05-14 | All three spec values (left/center/right) verified via wrapped-multi-line text. ⚠️ Single-line alignment within a wider container is not visible — SwiftUI's `multilineTextAlignment` only positions wrapped lines; without a `.frame(maxWidth: .infinity)` chain the Text shrinks to its intrinsic width. Documented in "Documented limitations". |
...
| `textAlign` | Single-line text alignment is not visually applied within a wider container. CSS expects `text-align: center/right` to position a single line within the element's content box, but JoyDOM-swift's `applyVisual` applies only `.multilineTextAlignment(_)` — SwiftUI's `multilineTextAlignment` controls how *wrapped* lines align relative to each other, not how a single-line `Text` is positioned inside its parent frame. Wrapped multi-line text (`textAlign/wrapped-multi-line.json`, `with-width-constrained.json`, `class-selector.json`, etc.) renders alignment correctly; single-line examples appear left-aligned regardless of declared value. Regression seam: `typography/text-align/single-line.json`. | A minimum-viable fix would chain `.frame(maxWidth: .infinity, alignment: ...)` alongside `.multilineTextAlignment(_)` in `JoyDOMView.applyVisual`, but the frame modifier would also expand non-text views that happen to inherit `textAlign`, potentially regressing flex-sizing for divs that opt into the property via inheritance. Deferred pending a targeted refactor that distinguishes text-leaf nodes (`p`, `h1`–`h6`, `span`, `primitive_string`) from layout containers before applying the frame. | textAlign coverage walk (this PR) |
